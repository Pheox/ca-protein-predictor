...
- ukoncit EA poctom generacii alebo pozadovanou presnostou!
*/
...
public class SSPEA {
    ...
    public CARule evolve() throws Exception{
        ...
        FitnessFunction myFF = new SimpleFF(data);
        conf.setFitnessFunction(myFF);

        // kde ukladat velkost okolia? do config. suboru ?
        Gene[] sampleGenes = new Gene[1 + (SSPEA.NEIGH*2 + 1)];


        sampleGenes[0] = new IntegerGene(conf, 0, SSPEA.MAX_STEPS);

        for (int i = 1; i-1 < SSPEA.NEIGH*2 + 1; i++) {
        ...
        for (int i = 0; i < SSPEA.MAX_GENERATIONS; i++) {
            logger.info(i + ". generation");
            population.evolve();
         }

         return CARule.fromChromosome(population.getFittestChromosome());
    }
}
